...
TEST_F( TCPClientListenerTest, setPriority ) {
    // ARRANGE
    system_context->register_setsockopt_impl([](int sockfd, int level, int optname, const void *optval, socklen_t optlen) { 
        return 0; 
    });
    ...
    // ACT
    int result = listener.setSocketPriority(5);

    // ASSERT
    #ifdef __APPLE__
    // Always fails on MacOS
    EXPECT_EQ(-1, result);
    #else 
    EXPECT_EQ(0, result);
    #endif
...
TEST_F( TCPClientListenerTest, setPriorityFails ) {
    // ARRANGE
    system_context->register_setsockopt_impl([](int sockfd, int level, int optname, const void *optval, socklen_t optlen) { 
        errno = EINVAL;
        return -1; 
...
TEST_F( TCPClientListenerTest, uninitialized ) {
    // ARRANGE
    system_context->register_setsockopt_impl([](int sockfd, int level, int optname, const void *optval, socklen_t optlen) { 
        errno = EINVAL;
        return -1; 
