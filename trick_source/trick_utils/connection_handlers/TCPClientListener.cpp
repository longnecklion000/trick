...
int Trick::TCPClientListener::setSocketPriority(unsigned short priority) {
    ...
    #ifdef __APPLE__
    // MacOS does not provide an SO_PRIORITY option or anything analagous
    std::cerr << "Socket Priority cannot be set on MacOS." << std::endl;
    return LISTENER_ERROR;

    #else
    if (_system_interface->setsockopt(_listen_socket, SOL_SOCKET, SO_PRIORITY, &priority, sizeof(priority)) < 0) {
        std::string error_message = "Unabled to set socket fd " + std::to_string(_listen_socket) + " to priority " + std::to_string(priority);
        perror(error_message.c_str());
        return LISTENER_ERROR;
    }
